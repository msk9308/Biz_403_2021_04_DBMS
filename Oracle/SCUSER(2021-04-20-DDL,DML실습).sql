...
-- TABLE
-- DBMS 를 사용하여 데이터를 추가하면
-- 마치 excel sheet에 데이터를 저장하는 것처럼
-- 데이터가 저장된다 
-- 실제 DBMS 소프트웨어가 저장하는 방식은 고유한 기술이 적용되어있지만
-- DBMS소프트웨어를 사용하는 사용자는 모든 데이터는
...
--      각 데이터의 이름을 지정하고, 각데이터의 형식(type)을 지정하여
--      표를 작성한다.


-- 데이터 Type
-- 문자열 데이터
--      CHAR : 저장되는 데이터의 길이가 (규칙으로)정해진 경우. 일정한 경우
--              데이터의 길이가 모두 같은 경우
--                  "고정길이 문자열"
--      VARCHAR : 저장되는 데이터의 최대크기만 지정 하고
--                  최대 크기보다 짧은 데이터를 저장할 수 있다
--                  최대 크기보다 짧은 데이터를 저장하면
--                  자동으로 공간을 축소하여 데이터를 저장한다
--                  "가변길이 문자열"
--      nVARCHAR : 한글과 같은 UNICODE문자열을 저장할 때
--              영문 알파베과 저장하는 방식이 달라서
--              문제를 일으킬 수 있다
--              한글과 같은 UNICODE데이터를 저장하는 칼럼은
--              nVARCHAR 키워드를 사용한다.
...
-- DML(Data Manipulation Language. 데이터조작어,데이터관리)
