...
--개수,    합산, 평균 ,최저, 최대
--통계 함수를 사용할 때 통계에 포함하지 않는 칼럼을
--보고자 할대는 GROUP BY를 사용하여 묶어줘야 한다

--학번으로 묶어서,동일한 학번의 국어점수 합계를 계산할때
...
SELECT SC.sc_num,
        ...
        SC.sc_eng,
        SC.sc_math
FROM tbl_Score ST, tbl_student ST
WHERE sc_num = st_num;
...
--학생데이터에 없는 학생의 성적이추가되어 있는지 여부를
--알아보고 싶을때 참조무결성에 오류가 있는지 알고 싶을때 
SELECT COUNT(*)
FROM tbl_score
...
--학생 데이터와 성적 데이터간의 참조무결성에 오류가 있기 때문에
--실제 학생데이터에 존재하는 성적정보만 보고 싶을 때
SELECT COUNT(*)
FROM tbl_student
    LEFT JOIN tbl_score
        ON st_num = sc_num;
--ROUND() DMBS 시스템함수, 소수점이하 자릿수제한
...
--각 학과의 총점을 계산
--평균도 계산
--학과 칼럼을 GROUP BY로 묶지 않으면 오류가 발생
SELECT 학과,SUM(총점) AS 학과총점, ROUND (AVG(평균),0) AS학과평균
FROM view_성적정보
GROUP BY 학과
