-- IOUSER 권한으로 접속
-- IOUSER는 DBA권한을 갖고 있기 때문에
-- 일반적인 표준 DDL, DML, DCL 명령들을 사용 할 수 있다
-- DDL(Data Definition Lang. 데이터 정의어)
--     객체생성 : CREATE TABLE, INDEX, VIEW 생성
--     객체삭제 DROP TABLE, VIEW< INDEX
--     객체변경(수정) TABLE, VIEW, INDEX
-- ORACLE 전용 DDL
...
--데이터가 INSERT 되어버렸다
--나중에 확인을 해보니 상품명이 없어서 데이터활용 가치가
--매우 떨어지는 현상이 발생했다
--이런상황을 INSERT(추가,삽입)이상현상이 발생했다
--  무결성이 훼손되었다

--기존의 table을 제거하고 무결성을 유지하기 위한 제약조건을
--"제약조건을 설정"하여 Table을 다시만들자 
...
SELECT io_date AS 거래일자,
    ...
    INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','성춘향','짜파게티',10,1100);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','이몽룡','신라면',20,1200);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','임꺽정','진라면',30,900);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','홍길동','매일우유',10,1600);

SELECT * FROM tbl_iolist;

--1. tbl_iolist로 부터 데이터를 가져오기
--2. 가져온 데이터중에서 io_buyer 칼럼에 저장된 값이 '홍길동' 인 데이터만
--      간추려서
--3. 보여달라
SELECT * FROM tbl_iolist WHERE io_buyer = '홍길동';
--tbl_iolist에 저장되어 있는 데이터 리스트중에서
--io_buyer 칼럼의 값이 '홍길동'인 데이터만 간추려서
--io_buyer,io_pname 칼럼 보이고 나머지 칼럼은 숨김으로 하여 보여달라
