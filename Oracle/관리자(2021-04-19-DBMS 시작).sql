...
--    oracle 이외의 다른 DBMS 소프트웨어에서는 데이터 저장소를
--        DataBase라고 한다.
-- 생성하기 : CREATE
-- 로컬디스크 oraclexe/data폴더에 mydb.dbf라는 파일을 생성하고 
-- mydb라는 이름으로 사용하겠다 
...
-- 나.Oracle DBMS서버는 명령을 수신한 후 문법검사를 하고
...
-- 다른 사용자는 TableSpace에 데이터를 추가,수정,삭제,조회하는데 제한을 둘수 있다.
-- 사용자를 등록할 때 사용자만의 TableSpace를 지정하여 줄 수 있다.
-- 사용자만의 데이터를 별도로 관리 할 수 있다.
-- 일반적인 DBMS는 데이터 저장공간을 DataBase라는 것 만으로 지정을 한다.
-- 만약 데이터를 추가하고자 할 때 어떤 DataBase에 데이터를 저장할 것인지
--  지정하지 않으면 명령수행에 문제가 발생할 수 있다.

-- 오라클은 User와 TableSpace를 연결하여 사용자가 DataBase처럼 
--  작동하게 할 수 있다.
-- 특정사용자로 접속만 하면 자동으로 지정된 TableSpace가 사용가능한
-- 상태가 되어 어떤 TableSpace에 데이터를 저장할 것인지 신경쓰지 않아도 된다.
