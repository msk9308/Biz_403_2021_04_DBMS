...
-- 명령이 실패한다 
-- 대량의 데이터가 저장된 TABLE을 변경한 경우는
-- 매우 신중하게 실행을 해야한다
-- 또한 미린 데이터 검증을 통하여 제약조건에 위배되는 
-- 데이터가 있는지 확인을 해야 한다 
...
저자 항목(칼럼)을 보면 저자가 2명이상인 데이터가 있고
또한 역자도 2명 이상인 경우가 있다 

데이터를 저장할 칼럼을 크게 설정하여 입력(IMPORT)하는데는
문제가 없는데 저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여
조회를 하려고 하면 문제가 발생할 수 있다 
특히 저자 이름으로 GROUPING을 하여 데이터를 조회 해보려고하면
상당히 어려움을 겪을 수 있다
*/

DESC tbl_books;
ALTER TABLE tbl_books --tbl books 테이블을 변경하라
RENAME COLUMN bk_author TO bk_author1; --bk_author 칼럼을 bk_thor1으로 변경하라
...
한 칼럼에 2개 이상의 데이터가 구분자(,)로 나뉘어서 저장되는 것을 
    막는 조치 
    
이미 2개이상의 데이터가 저장된 경우 분리하여 원자성을 
갖도록 하는것이다 
...
2. 도서 정보와 관련된 저자 데이터를 저장할 Table을 새로 생성한다
    ...
    "도서의 ISBN"과 "저자리스트"를 포함하는 형태의 데이터를 만든다
    -----------------------------------------------------------
    ISBN        저자
    -----------------------------------------------------------
    1           홍길동
...
--저자가 2명 이상인 경우는 같은 ISBN,같은 도서명,다른저자 형식으로
--저자 수 만큼 데이터가 출력된다
