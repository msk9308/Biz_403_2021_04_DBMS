...
-- DBA (DataBaseAdministrator)의 권한을 갖는 사용자가 사용하는 명령어
...
-- SYSDBA는 시스템 모든 부분에 접근할수 있는 권한이 있어서
-- 자칫 명령을 잘못 입력하고 실행하면 Oracle DBMS자체에 문제가 생긴다
...
-- 세부적인 항목은 회사, 조직마다 별도러 정책을 만들어 사용하기 때문에 
-- 규칙이 정해진 것은 없다.
...
-- CREATE SESSION : DBMS 서버에(logon)을 할 수 있는 권한
...
-- tbl_school TABLE에 데이터를 조회할 수 있는 권한을
-- scuser에게 부여
