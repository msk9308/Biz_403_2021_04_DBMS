...
--SQL Developer에서
--데이터를 INSERT(추가), UPDATE(변경) , DELETE(삭졔) 명령을 수행하면
--OracleServer와 SQL Developer 사이에서 중간에
...
--옵션항목이 WHERE절을 반드시 포함하자
--또한 특별하게 필요한 경우가 아니면
--WHERE절에 포함하는 조건은 PK칼럼을 기준으로 하자 
DELETE FROM tbl_score WHERE sc_num = 'S0001';
UPDATE tbl_score SET sc_kor = 100 WHERE sc_num = 'S0001';
...
--VIEW 만들 떄 설정한 AS값들이 칼럼이름으로 사용된다
SELECT * FROM view_score;
SELECT * FROM view_score WHERE 총점 < 200;
