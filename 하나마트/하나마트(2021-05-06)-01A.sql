...
--농사를 지어서 쌀 20Kg를 생산하여 판매를하면
--5만원 정도의 금액에 판매하게 된다
--쌀을 공장에서 가공하여 생산품(공산품)으로 만들게되면
--실제 20kg의 쌀을 직접 판매하는것보다 더 비싼 가격에 판매하게 된다 
--이 때 실제 쌀보다 더 많은 이익이 발생하게 되므로
--가치가 부가 되었다라고 표현한다
...
--       매출금액은 VAT 포함된 금액이다
--매입과 매출 데이터로 지난 1년간 납부한 VAT금액을 계산해보자
--매입금액 : 22737397
--매출금액 : 41683800
...
--거래처별로 매입과 매출 합계
--DECODE(칼럼명, 조건값, true일 때, false일 때)
...
--전체데이터에서 상품리스트만 중복없이 조회
--단가 데이터 중에서 제일 높은 단가 가져오기
SELECT io_pname,
...
4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
 ...
 E2 항목의 값을 0.8로 곱하면 E2의 80% 가격이 되고
...
매입매출 데이터와, 상품정보, 거래처정보를 
JOIN하기 위해서는 매입매출데이터에 상품코드, 거래처코드가 
있어야 한다.
그러나 현재 데이터를 코드 칼럼이 없이 이름 칼럼만 있는 상태

매입매출 데이터에 상품코드, 거래처코드 칼럼을 추가하고 
세 테이블을 JOIN할 수 있도록 변경하기 
*/
