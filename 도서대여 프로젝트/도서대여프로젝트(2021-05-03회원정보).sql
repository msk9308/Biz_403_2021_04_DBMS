...
오라클에서는 일련번호를 생성하기 위한별도의 객체가 존재한다
...
--SEQ의 START값을 적용하는 테이블의 seq최대값보다 크게
--설정을 하자 
...
VALUES(seq_book_rent.NEXTVAL, '2021-04-20','9791188850440','B0020');
...
--FK 관계가 설정되지 앟은 TABLE 간에서는 조회되는 데이터가
--실제와 다를 수 있다 //반드시 FK가 설정되어 있어야한다
SELECT 
    BR.br_sdate AS 대여일,
    BR.br_bcode AS 회원코드,
    BU.bu_name AS 회원명,
...
--FROM절에 표현된 Table 데이터 위주로 조회하고자 할 때
--FK가 설정되지 않아도 데이터조회는 모두 할 수 있다
...
FROM view_도서대여정보 BR
    LEFT JOIN tbl_buyer BU
        ON BR.회원코드 = BU.bu_code
        WHERE 대여일 < '2021-04-25' AND 반납일 IS NULL
        GROUP BY 대여일, 회원코드, 회원명, 반납일, BU.bu_tel;
...
VALUES (seq_book_rent.NEXTVAL,
            '2021-04-01','B0012','9791162540527');
