-- bookUser접속
-- books Table과 author, company table은 Realation 관계가 있다
--   books의 bk_ccode와 company의 cp_code
--   books의 bk_acode와 company의 au_code
-- 연관 관계에 있는 table을 EQ JOIN을 실행할 때
-- 만약 author, company table에 없는 데이터(코드)가 
-- books에 있다면 EQ JOIN을하면 데이터가 누락되어 버린다
-- 또한 LEFT JOIN을 하면 값이(null)로 출력된다
-- JOIN된 데이터가 누락되거나 (null) 출력되는 것은
-- 데이터에 문제가 발생한 것이다. "참조 무결성이 무너졌다"라고한다
...
-- JOIN결과가 잘못되면 DB신뢰성에 매우 큰 문제가 발생한다
...
-- 출판사 정보를 연동 
-- tbl_company를 parent라고하며 REFERENCES table 이라고 한다
