...
현재 매입매출 테이블엔 상품이름과 거래처명(대표포함)이
저장되어 있다 
...
--  전체 데이터 상품별로 가장 높은 가격을 가져와서 매입 매출 단가로 사용하자
SELECT io_pname, 
...
매입단가가 0인 경우 매출단가에서 마진(20%)을 빼고,
다시 부가세(10%)뺀 가격으로 계산
매입단가 = (매출단가 / 1.2)/1.1

매출단가는 매입단가에 20% 마진추가, 부가세 10% 추가
매출단가 = (매입단가 * 1.2) * 1.1
